...
// to the current build's content hash (a3fa00e4 as of ADR-19's
// verification) -- regenerating with a different OCP structure changes
// that hash, and this file's function names need updating to match. Not
// hidden behind a macro on purpose: the coupling is real, and pretending
// otherwise would just make the next person rediscover it the hard way.
//
...
#include "acados_solver_ocp_lateral_mpc_kinematic_bicycle_a3fa00e4.h"

#define NX OCP_LATERAL_MPC_KINEMATIC_BICYCLE_A3FA00E4_NX
#define NU OCP_LATERAL_MPC_KINEMATIC_BICYCLE_A3FA00E4_NU
#define NP OCP_LATERAL_MPC_KINEMATIC_BICYCLE_A3FA00E4_NP
#define NBX0 OCP_LATERAL_MPC_KINEMATIC_BICYCLE_A3FA00E4_NBX0
...
int main(int argc, char **argv) {
    ...
    auto *capsule = ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_create_capsule();
    int N = OCP_LATERAL_MPC_KINEMATIC_BICYCLE_A3FA00E4_N;
    int status = ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_create_with_discretization(
        capsule, N, nullptr);
    if (status) {
    ...
    ocp_nlp_config *nlp_config =
        ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_get_nlp_config(capsule);
    ocp_nlp_dims *nlp_dims =
        ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_get_nlp_dims(capsule);
    ocp_nlp_in *nlp_in = ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_get_nlp_in(capsule);
    ocp_nlp_out *nlp_out = ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_get_nlp_out(capsule);
    ...
    for (int i = 0; i <= N; i++) {
        ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_update_params(capsule, i, p, NP);
    }
    ...
    status = ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_solve(capsule);
    ...
    ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_free(capsule);
    ocp_lateral_mpc_kinematic_bicycle_a3fa00e4_acados_free_capsule(capsule);
