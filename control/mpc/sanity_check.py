...
of fixed (c0, c1, c2, v) cases, and computes the explicit numeric
criteria requested rather than an eyeballed verdict: does the controller
steer toward the reference (e_lat/e_psi magnitude decreasing over the
horizon, not just "delta has some sign"), does anything saturate on a
small offset (miscalibrated-weight symptom), does left/right mirror
correctly, does a constant-curvature case's terminal delta approach the
geometric feedforward atan(L*kappa).
...
def report_case(name, c0, c1, c2, v):
    ...
    print(f"\n=== {name} ===")
    print(f"  c0={c0:+.4f}  c1={c1:+.4f}  c2={c2:+.6f}  v={v:.2f}   status={status}")
...
def main():
    ...
    cases = [
        ...
        ("2. tight arc away-from-join (R=3m)", 0.0, 0.0, kappa_r3_away / 2, 1.0),
        ("3. tight arc near-join (R=3m blend)", 0.0, 0.0, kappa_r3_near / 2, 1.0),
        ...
        ("5b. heading offset -0.1 rad (mirror of 5a)", 0.0, -math.tan(0.1), 0.0, 1.0),
        ("6. combined offset + curvature", 0.1, math.tan(0.1), kappa_r3_away / 2, 1.0),
    ]
    ...
    for name in ["4a. lateral offset +0.1 m", "4b. lateral offset -0.1 m (mirror of 4a)",
                 "5a. heading offset +0.1 rad", "5b. heading offset -0.1 rad (mirror of 5a)",
    ...
    for pair in [("4a. lateral offset +0.1 m", "4b. lateral offset -0.1 m (mirror of 4a)"),
        ...
        print(f"    max|delta_traj_a + delta_traj_b| across horizon = {traj_diff:.8f}")

    print("\n-- curvature steady-state feedforward: does terminal delta "
          "approach atan(L*kappa), not zero? (the actual test of ADR-19's "
          "formulation reversal) --")
    for name, kappa in [("2. tight arc away-from-join (R=3m)", kappa_r3_away),
                         ("6. combined offset + curvature", kappa_r3_away)]:
        r = results[name]
        delta_ff = math.atan(L * kappa)
        delta_end = r["delta_traj"][-1]
        print(f"  {name}: kappa={kappa:.5f}  atan(L*kappa)={delta_ff:.6f} rad  "
              f"delta_end={delta_end:.6f} rad  diff={abs(delta_end-delta_ff):.6f} "
              f"({abs(delta_end-delta_ff)/delta_ff*100:.1f}% of feedforward value)")
