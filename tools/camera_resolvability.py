"""How far ahead can this camera actually resolve the lane? Ground
distance compresses into fewer and fewer image rows as it recedes
toward the horizon, and the lane's projected width in pixels shrinks
with it -- past some distance, a point counted as "visible" by
camera_visibility.py carries near zero usable information: a handful
of pixels the CNN cannot regress a lateral offset from, or a few
points too close together in image space to represent a curve instead
of noise.

This script measures three independent limits and reports the
tightest:

  L_resolvable   -- projected lane width drops below 10 px at CNN
                    input resolution (after crop/resize, from
  ...
  L_representable -- from track geometry alone: where the quadratic
                    y(x) = c0 + c1 x + c2 x^2 preview path (the
                    /lane_state contract's representation, see
                    docs/lane-state-contract.md section 1) stops being
                    a good model of a circular arc of radius R_min,
                    taken as tan(s/R_min) > 1.0

  L_usable = min(L_resolvable, L_separable, L_representable)

Also sweeps camera mount height (0.125 m current, 0.25 m) x pitch
(17.5 deg current, 10 deg) to show the resolution/L_usable trade --
entirely local to this script; camera_visibility.py's actual constants
are never touched (measure and report, decisions come after).
...
from perception.dataset.track_definitions import (  # noqa: E402
    RADIUS_1, RADIUS_2, LANE_HALF_WIDTH,
)
...
def project_straight_ahead(
        dist, lateral, height, pitch_down, fovy_deg=CAM_FOVY_DEG,
...
def compute_curves(height, pitch_down):
    v = np.array([
        project_straight_ahead(d, 0.0, height, pitch_down)[1]
        for d in D_GRID])
    u_left = np.array([
        project_straight_ahead(d, LANE_HALF_WIDTH, height, pitch_down)[0]
        for d in D_GRID])
    u_right = np.array([
        project_straight_ahead(d, -LANE_HALF_WIDTH, height, pitch_down)[0]
        for d in D_GRID])
...
def resolvability_limits(height, pitch_down, crop_cfg):
    ...
    l_resolvable = first_crossing_below(
        D_GRID[valid], lane_width_cnn_px[valid], LANE_WIDTH_PX_THRESHOLD)

    # rows_per_metre * GAP < ROWS  <=>  rows_per_metre < ROWS / GAP
    rpm_threshold = SEPARABILITY_ROWS / SEPARABILITY_GAP_M
    l_separable = first_crossing_below(
        D_GRID[valid], rpm[valid], rpm_threshold)

    return {
        "v": v, "lane_width_px": lane_width_px,
        "lane_width_cnn_px": lane_width_cnn_px,
...
def main():
    ...
    for height, pitch, label in sweep_points:
        ...
        l_use = min(x for x in (
            res["L_resolvable"], res["L_separable"], l_representable)
            if x is not None)
        sweep_rows.append((
            label, height, math.degrees(pitch), res["L_resolvable"],
            res["L_separable"], l_representable, l_use))
    ...
    ax.axhline(
        SEPARABILITY_ROWS / SEPARABILITY_GAP_M, color="r", linestyle="--",
        label=f"{SEPARABILITY_ROWS:.0f} rows / {SEPARABILITY_GAP_M} m "
              f"threshold")
    ax.set_xlabel("ground distance [m]")
    ...
    ax = axes[1, 0]
    ax.plot(
        D_GRID[valid], current["lane_width_px"][valid],
        label="320x240 (native)")
    ax.plot(
        D_GRID[valid], current["lane_width_cnn_px"][valid],
        label="CNN input (crop+resize)")
    ax.axhline(
        LANE_WIDTH_PX_THRESHOLD, color="r", linestyle="--",
    ...
    ax.axhline(
        l_representable, color="k", linestyle=":",
        label="L_representable (fixed)")
    ax.set_xticks(x)
    ...
    lines.append(
        "Generated by `tools/camera_resolvability.py`. Measures how far "
        "ahead the camera can actually resolve the lane, independent of "
        "whether `camera_visibility.py` counts a point as \"visible\" -- "
        "see `LOOKAHEAD_M` vs these limits.\n"
    )
    lines.append("## Current camera (height=0.125 m, pitch=17.5 deg)\n")
    if current["L_resolvable"] is not None:
        lines.append(
            f"- **L_resolvable** (lane width < "
            f"{LANE_WIDTH_PX_THRESHOLD:.0f} px at CNN input resolution): "
            f"**{current['L_resolvable']:.2f} m**\n")
    else:
        lines.append("- **L_resolvable**: not reached within the 20 m sweep\n")
    if current["L_separable"] is not None:
        lines.append(
            f"- **L_separable** (two points {SEPARABILITY_GAP_M} m apart "
            f"fall under {SEPARABILITY_ROWS:.0f} image rows apart): "
            f"**{current['L_separable']:.2f} m**\n")
    else:
        lines.append("- **L_separable**: not reached within the 20 m sweep\n")
    lines.append(
        f"- **L_representable** (tan(s / R_min) > 1, R_min = {r_min:.1f} "
        f"m): **{l_representable:.2f} m**\n")
    ...
    for d in [0.25, 0.5, 1, 2, 3, 5, 8, 12, 16]:
        ...
        if valid[idx]:
            lines.append(
                f"| {d:g} | {current['lane_width_px'][idx]:.1f} | "
    ...
    lines.append(
        "| mount | L_resolvable [m] | L_separable [m] | "
        "L_representable [m] | L_usable [m] |\n|---|---|---|---|---|\n")
    for label, height, pitch_deg, lr, ls, lrep, lu in sweep_rows:
        ...
        lines.append(
            f"| {label} | {lr_s} | {ls_s} | {lrep:.2f} | {lu:.2f} |\n")
