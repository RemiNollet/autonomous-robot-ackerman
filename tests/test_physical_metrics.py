...
from perception.model.physical_metrics import (  # noqa: E402
    format_physical_report, physical_metrics,
)
from perception.model.targets import (  # noqa: E402
    E_PSI_SCALE, E_Y_SCALE, KAPPA_SCALE,
)
...
class _PerfectModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        self.idx += n
        conf_logit = torch.where(
            v > 0.5, torch.full_like(v, 10.0), torch.full_like(v, -10.0))
...
def test_invalid_samples_excluded_from_e_y_e_psi_but_counted_in_confidence():
    ...
    targets = torch.tensor(
        [[rng.uniform(-0.3, 0.3) / E_Y_SCALE,
          rng.uniform(-0.4, 0.4) / E_PSI_SCALE, 0.0] for _ in range(n)],
        dtype=torch.float32,
    )
    ...
    m = physical_metrics(model, loader, torch.device("cpu"))
    assert m["e_y"]["n"] == 7
    assert (m["confidence"]["n_valid"] == 7
            and m["confidence"]["n_invalid"] == 3)
...
def test_curvature_bin_breakdown_separates_bins_correctly():
    ...
    m = physical_metrics(model, loader, torch.device("cpu"))
    total_binned = sum(
        m["e_y_by_bin"][label]["n"] for label, _ in
...
def test_format_physical_report_runs_and_mentions_kappa_is_not_reported():
    ...
    m = physical_metrics(model, loader, torch.device("cpu"))
    report = format_physical_report(
        m, title="test run", in_distribution=True,
