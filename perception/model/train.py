"""Training entry point for LaneCNN (M2). Plain PyTorch: Adam, cosine
schedule, argparse. No Lightning, no Hydra, no config-class hierarchy.

Hyperparameters default from perception/model/training_config.yaml
(the versioned, comparable source of truth -- its hash is written into
every row of results.md); CLI flags override individual values for a
one-off run.

Also runs the M2 baselines/ablation (these are the point of the
exercise, not extras -- see docs/decisions.md ADR-11):
  1. constant predictor       -- is the CNN better than memorizing the
                                  mean?
  2. width_mult 1.0 vs 0.25   -- does v0 (ADR-11 findings 3-5) actually
                                  need full capacity, or is the margin
                                  deliberate over-provisioning for the
                                  eventual degraded-domain case?
  3. mirror-generalization probe -- train on source renders only,
                                  evaluate on their mirror twins
                                  (ADR-10) -- the only v0 evaluation
                                  that isn't pure interpolation within
                                  identical geometry (ADR-11 finding 5).

Usage:
    python3 perception/model/train.py             # full run
...
from perception.model.lane_cnn import (  # noqa: E402
    LaneCNN, count_params, count_macs,
)
from perception.model.physical_metrics import (  # noqa: E402
    physical_metrics, format_physical_report,
)
from perception.model.loss import lane_loss, component_losses  # noqa: E402
from perception.model.targets import (  # noqa: E402
    E_Y_SCALE, E_PSI_SCALE, KAPPA_SCALE,
)
...
def _kappa_intro_paragraph(config: dict) -> str:
    ...
    if weight == 0:
        return (
            ...
            "kappa label is measurably wrong within L_usable (2.36m) of "
            "a curvature transition (~42% of the loop). "
            "perception/model/kappa_transition_proximity.png: on "
            "straight samples, mean|kappa_pred| correlates at r=-0.67 "
            "with distance to the next transition, plateauing almost "
            "exactly at L_usable (0.023 beyond it vs 0.115 within it, "
            "a 5x gap) -- the network is reading the road correctly "
            "and being penalised for it. The MAE/scatter numbers below "
            "are NOT merely unusable: a kappa output that predicts "
            "curvature on straights and near-zero in curves would "
            "actively steer an MPC feedforward term off a straight "
            "line approaching a bend. The output head stays in the "
            "architecture (component_losses still logs kappa's raw, "
            "unweighted loss in loss_curves.png for monitoring) so a "
            "future windowed or continuous-curvature label can reuse "
            "it (ADR-14).\n"
        )
    return (
        f"\nkappa's loss weight is {weight} (ADR-18): the point-wise "
        "Frenet label's near-transition problem (ADR-14) was addressed "
        "by relabeling data/dataset_v0/labels.csv with "
        "perception/dataset/windowed_relabel.windowed_curvature_average "
        "-- a weighted average of true track curvature over the "
        "camera's actual visible window, not a Cartesian polynomial "
        "fit (which was tried first and rejected: 13-41% biased even "
        "on a pure arc with zero discontinuity present, see "
        "windowed_relabel.py's module docstring). Near-join vs "
        "away-from-join kappa MAE, and the check that "
        "e_y/e_psi/confidence didn't regress from retraining, are in "
        "perception/model/analyze_adr18_retrain.py's output and "
        "ADR-18 (docs/decisions.md) -- not in the per-sample table "
        "below, which has no notion of distance-to-transition.\n"
    )
...
def _kappa_note(config: dict) -> str:
    ...
    kappa_weight = config["component_weights"]["kappa"]
    if kappa_weight == 0:
        return None
    return (
        f"**kappa: loss weight {kappa_weight}, trained.** "
...
def _physical_mae(
        pred3_n: torch.Tensor, target3_n: torch.Tensor,
        valid: torch.Tensor):
    ...
    scale = torch.tensor(
        [E_Y_SCALE, E_PSI_SCALE, KAPPA_SCALE], device=pred3_n.device)
...
def evaluate(
    ...
    for img, target, valid in loader:
        ...
        pred, _ = model(img)
        loss, reg, conf = lane_loss(
            pred, target, valid, component_weights=component_weights)
    ...
    return EpochStats(
        loss=total_loss / n_batches, reg_loss=total_reg / n_batches,
        conf_loss=total_conf / n_batches,
        mae_e_y=mae[0], mae_e_psi=mae[1], mae_kappa=mae[2],
        conf_accuracy=correct_conf / max(n_total, 1),
        comp_e_y=comp_sums["e_y"] / n_batches,
        comp_e_psi=comp_sums["e_psi"] / n_batches,
        comp_kappa=comp_sums["kappa"] / n_batches,
        comp_conf=comp_sums["confidence"] / n_batches,
    )


def train_model(
        width_mult: float, train_ds, val_ds, epochs: int, lr: float,
    ...
    for epoch in range(epochs):
        ...
        # Full eval pass only over val (not train, which was already
        # scored live above) -- halves epoch cost, and reports genuine
        # train-mode (not eval-mode BatchNorm) loss, the more honest
        # number.
        val_stats = evaluate(
            model, val_loader, device, component_weights=component_weights)
        history.append({
            "epoch": epoch, "train_loss": train_loss,
            "val_loss": val_stats.loss,
            "train_e_y": train_comp["e_y"], "val_e_y": val_stats.comp_e_y,
            "train_e_psi": train_comp["e_psi"],
            "val_e_psi": val_stats.comp_e_psi,
            "train_kappa": train_comp["kappa"],
            "val_kappa": val_stats.comp_kappa,
            "train_confidence": train_comp["confidence"],
            "val_confidence": val_stats.comp_conf,
            "val_mae_e_y": val_stats.mae_e_y,
            "val_mae_e_psi": val_stats.mae_e_psi,
            "val_mae_kappa": val_stats.mae_kappa,
        })
        print(f"{log_prefix}epoch {epoch+1:2d}/{epochs}  "
              f"train_loss={train_loss:.4f}  "
              f"val_loss={val_stats.loss:.4f}  "
              f"comp[e_y={val_stats.comp_e_y:.4f} "
              f"e_psi={val_stats.comp_e_psi:.4f} "
              f"kappa={val_stats.comp_kappa:.4f} "
              f"conf={val_stats.comp_conf:.4f}]  "
              f"val_MAE e_y={val_stats.mae_e_y:.4f}m "
              f"e_psi={val_stats.mae_e_psi:.4f}rad "
              f"kappa={val_stats.mae_kappa:.4f}/m "
    ...
    final_val_stats = evaluate(
        model, val_loader, device, component_weights=component_weights)
    return model, final_val_stats, history


def constant_predictor_baseline(
        train_ds: LaneDataset, eval_ds: LaneDataset) -> EpochStats:
...
def save_checkpoint(model: nn.Module, width_mult: float, path: str):
    ...
    torch.save(
        {"state_dict": model.state_dict(), "width_mult": width_mult}, path)


def plot_loss_curves(history: list, path: str):
    # Agg must be selected before pyplot is imported anywhere in the
    # process (headless-safe backend), so both stay local to this call.
    import matplotlib
    matplotlib.use("Agg")
    ...
    fig.suptitle(
        "Per-component loss (normalized units for e_y/e_psi/kappa; "
        "BCE for confidence)")
    fig.tight_layout()
    os.makedirs(os.path.dirname(path), exist_ok=True)
...
def plateau_epoch(
        history: list, key: str, rel_tol: float = 0.02,
        window: int = 5) -> int:
...
def main():
    ...
    ap.add_argument(
        "--plot-main-loss-curves", action="store_true",
    ...
    train_ds = LaneDataset(
        labels_csv, img_dir, split="train", augment=True, seed=args.seed,
    ...
    model, val_stats, history = train_model(
        width_mult=1.0, train_ds=train_ds, val_ds=val_ds,
        epochs=args.epochs, lr=args.lr, batch_size=args.batch_size,
        device=device, seed=args.seed,
        component_weights=config["component_weights"], log_prefix="[main] ",
    )
    test_stats = evaluate(
        model, DataLoader(test_ds, batch_size=args.batch_size), device,
    ...
    plateaus = {
        k: plateau_epoch(history, f"val_{k}")
        for k in ("e_y", "e_psi", "kappa", "confidence")}
    print("plateau epochs (val, main model):", plateaus)

    physical_sections = []
    main_physical = physical_metrics(
        model, DataLoader(test_ds, batch_size=args.batch_size), device)
    physical_sections.append(format_physical_report(
        main_physical, title="main (width=1.0), test split",
        in_distribution=True, lane_half_width=LANE_HALF_WIDTH,
        heading_envelope=POS_HEADING_RANGE,
    ...
    results["constant predictor"] = {
        "params": 3, "macs": 0, "val": None, "test": const_stats,
        "train_time_s": 0.0}

    # --- baseline 2: width ablation, width_mult=0.25 ---
    t0 = time.time()
    model025, val_stats025, _ = train_model(
        width_mult=0.25, train_ds=train_ds, val_ds=val_ds,
        epochs=args.epochs, lr=args.lr, batch_size=args.batch_size,
        device=device, seed=args.seed,
        component_weights=config["component_weights"],
        log_prefix="[width=0.25] ",
    )
    test_stats025 = evaluate(
        model025, DataLoader(test_ds, batch_size=args.batch_size), device,
        component_weights=config["component_weights"])
    save_checkpoint(
        model025, 0.25, f"{CHECKPOINT_DIR}/lane_cnn_width0.25_best.pt")
    results["width ablation (width=0.25)"] = {
        "params": count_params(model025), "macs": count_macs(model025),
        "val": val_stats025, "test": test_stats025,
        "train_time_s": time.time() - t0,
    }

    # --- baseline 3: mirror-generalization probe ---
    train_src_ds = LaneDataset(
        labels_csv, img_dir, split="train", mirrored=False, augment=True,
        seed=args.seed, augment_params=aug_params)
    train_mirror_eval_ds = LaneDataset(
        labels_csv, img_dir, split="train", mirrored=True, augment=False)
    t0 = time.time()
    probe_model, probe_val_stats, _ = train_model(
        width_mult=1.0, train_ds=train_src_ds, val_ds=val_ds,
        epochs=args.epochs, lr=args.lr, batch_size=args.batch_size,
        device=device, seed=args.seed,
        component_weights=config["component_weights"],
        log_prefix="[mirror-probe] ",
    )
    mirror_loader = DataLoader(
        train_mirror_eval_ds, batch_size=args.batch_size)
    mirror_eval_stats = evaluate(
        probe_model, mirror_loader, device,
        component_weights=config["component_weights"])
    results["mirror-generalization probe"] = {
        "params": count_params(probe_model), "macs": count_macs(probe_model),
        "val": probe_val_stats, "test": mirror_eval_stats,
        "train_time_s": time.time() - t0,
        "note": "trained on train-split SOURCE renders only; 'test' "
                "column here is evaluation on those same samples' "
                "MIRROR twins (unseen), not the usual test split",
    }
    mirror_physical = physical_metrics(probe_model, mirror_loader, device)
    physical_sections.append(format_physical_report(
        mirror_physical,
        title="mirror-generalization probe, mirror-twin eval set",
        in_distribution=False, lane_half_width=LANE_HALF_WIDTH,
        heading_envelope=POS_HEADING_RANGE,
        kappa_note=_kappa_note(config),
    ))
...
def _write_results(
        results: dict, config: dict, run_hash: str, plateaus: dict = None,
        physical_sections: list = None):
    lines = [
        "# M2 results\n",
        f"\nconfig_hash={run_hash}  "
        f"(perception/model/training_config.yaml, "
        f"epochs={config['epochs']} lr={config['lr']} "
        f"batch_size={config['batch_size']} seed={config['seed']})\n",
        "\nNo early stopping on val loss: v0's val split does not "
        "measure generalisation (ADR-11 finding 5 -- every arc is "
        "geometrically identical to its twin), so stopping on it "
        "would halt on noise and make runs non-reproducible. Epoch "
        "count is fixed, chosen from where "
        "perception/model/loss_curves.png actually plateaus.\n",
        _kappa_intro_paragraph(config)]
    if plateaus:
        lines.append(
            f"\nPlateau epoch per component (val, main model): "
            f"{plateaus}\n")
    ...
    lines += [
        "\n| Run | Params | MACs | val loss | normalized test MAE e_y | "
        "normalized test MAE e_psi | conf acc | train time |\n",
        "|---|---|---|---|---|---|---|---|\n"]
    for name, r in results.items():
        val_loss = f"{r['val'].loss:.4f}" if r["val"] is not None else "n/a"
        t = r["test"]
        lines.append(
            f"| {name} | {r['params']:,} | {r['macs']:,} | {val_loss} | "
    ...
    if physical_sections:
        ...
        lines.append(
            "\nSee `perception/model/physical_metrics.py`. "
            "\"Normalized\" above is the same unit the loss is computed "
            "in (dimensionless, scaled by envelope width); the tables "
            "below are the physical-unit numbers a controller would "
            "actually see.\n"
        )
