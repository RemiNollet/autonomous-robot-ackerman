...
onward under the point-wise label
(`loss_curves_ADR14_pointwise_kappa0.png` -- `loss_curves.png` itself gets
regenerated by every training run, so this is a frozen copy of what it
showed at the time this finding was made; the live file now shows ADR-18's
retrain instead, whose kappa validation loss converges cleanly, no
flatline -- see ADR-18) -- the network was reading the road correctly
...
(`T=8` transitions x `L_usable` / `total_length`). kappa's loss weight was
set to 0 as a result (ADR-14) -- not merely small: a kappa output that
predicts curvature on straights and near-zero in curves would actively
steer an MPC feedforward term off a straight line approaching a bend,
which is worse than publishing nothing.

**Update, ADR-18:** relabeling with a windowed curvature average (not a
Cartesian polynomial fit -- that was tried first and rejected, biased
13-41% even on a pure arc) instead of the point-wise projection fixes
...
- [`perception/model/kappa_transition_proximity.png`](model/kappa_transition_proximity.png) --
  the r=-0.67 evidence
- [`perception/model/loss_curves_ADR14_pointwise_kappa0.png`](model/loss_curves_ADR14_pointwise_kappa0.png) --
  per-component train/val loss, the overfitting signature on kappa under the
  point-wise label. `perception/model/loss_curves.png` itself is
  regenerated by every training run (currently ADR-18's retrain, which
  does NOT show this signature -- kappa's val loss converges cleanly under
  the windowed relabel); this frozen copy is the ADR-14 evidence
  specifically.
...
- [`perception/model/results.md`](model/results.md) -- full numeric results,
  including ADR-18's near-join/away-from-join kappa comparison
- [`docs/decisions.md`](../docs/decisions.md) -- ADR-7 through ADR-12,
  ADR-14 (kappa loss weight), ADR-18 (windowed-relabel retrain)
